...
st.set_page_config(
    layout="wide",
    page_title="SmartBelt · Vibration",
    page_icon="📡"
)
...
PLOTLY_LAYOUT = dict(
    ...
    legend=dict(bgcolor="rgba(0,0,0,0)", bordercolor="#2A2F45"),
...
with col_title:
    st.markdown("# 📡 Vibration Dashboard")
    st.markdown(
...
if uploaded_file:
    ...
    with st.sidebar:
        ...
        start_time = st.number_input("Start (s)", 0.0, duration_sec, 0.0)
        end_time   = st.number_input("End (s)",   0.0, duration_sec, min(40.0, duration_sec))
        if end_time - start_time > 40:
            end_time = start_time + 40

        st.markdown("## 📶 Sampling")
        override_fs = st.checkbox("Override fs")
        manual_fs   = st.number_input("Manual fs (Hz)", value=fs_auto)
        fs = manual_fs if override_fs else fs_auto
        st.caption(f"Active fs: **{fs:.1f} Hz**")

        ma_window = st.slider("MA Window", 1, 50, 5)

        st.markdown("## 〰 FFT")
        use_windowing = st.checkbox("Windowing")
        use_averaging = st.checkbox("Averaging")
        use_overlap   = st.checkbox("Overlap")
        window_type   = "hann"
        window_size   = 1024
        overlap_factor = 0.5
        ...
        with col_f1:
            fmin = st.number_input("f min", value=0.0)
        with col_f2:
            fmax = st.number_input("f max", value=50.0)

        st.markdown("## 🔺 Peaks")
        peak_height     = st.number_input("Min Height",     value=0.0)
        peak_distance   = st.slider("Min Distance", 1, 200, 20)
        peak_prominence = st.number_input("Min Prominence", value=0.0)

        st.markdown("## 📊 PSD")
        nperseg = st.slider("nperseg", 128, 4096, 1024)

        st.markdown("## 🌀 STFT")
        stft_window  = st.slider("Window size", 128, 4096, 512)
        stft_overlap = st.slider("Overlap %",   0,   90,   50)

        st.markdown("## 🔍 Anomaly")
        model_choice = st.selectbox("Model", ["Isolation Forest", "PCA"], index=1)
        contamination = st.slider("Contamination", 0.001, 0.1, 0.001, 0.001, format="%.3f")
        rms_percentile = st.slider("RMS Threshold %ile", 0, 80, 10)
        axis_choice    = st.selectbox("Axis", ["X (40g)", "Y (40g)", "Z (40g)", "All"])
    ...
    with tab2:
        ...
        st.dataframe(
            stats.style.format({c: "{:.4f}" for c in stats.columns if c != "Axis"}),
