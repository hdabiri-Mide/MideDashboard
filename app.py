...
def detect_channels(doc):
    ...
    meas_col = find_col(["measurement", "type"])
    unit_col = find_col(["unit"])

    display = pd.DataFrame(index=table.index)
    display["Name"] = table[name_col] if name_col else "Unknown"
    display["Measurement"] = table[meas_col] if meas_col else "Unknown"
    display["Units"] = table[unit_col] if unit_col else ""
...
if uploaded_file:
    ...
    def format_label(idx):
        row = table.loc[idx]
        return f"{idx} | {row['Name']} | {row['Measurement']} ({row['Units']})"
    ...
    acc_id = st.sidebar.selectbox(
        ...
        index=channel_ids.index(auto_acc) if auto_acc in channel_ids else 0
    )
    ...
    temp_id = st.sidebar.selectbox(
        ...
        options=channel_ids,
        format_func=format_label,
        index=channel_ids.index(auto_temp) if auto_temp in channel_ids else 0
    )
