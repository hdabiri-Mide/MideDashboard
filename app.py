...
def detect_channels(doc):
    ...
    for idx, row in table.iterrows():
        meas = str(row.get("Measurement Type", "")).lower()
        if "acceleration" in meas and acc_id is None:
            acc_id = idx
        if "temperature" in meas and temp_id is None:
            temp_id = idx

    return table, acc_id, temp_id
...
def detect_peaks(freqs, values):
    peaks, _ = find_peaks(values, height=peak_height,
...
if uploaded_file:
    ...
    acc_id = st.sidebar.selectbox(
        ...
        table.index,
        index=list(table.index).index(auto_acc) if auto_acc in table.index else 0
    )

    temp_id = st.sidebar.selectbox(
        "Temperature Channel",
        table.index,
        index=list(table.index).index(auto_temp) if auto_temp in table.index else 0
    )

    # ---- Load data ----
    acc_df = endaq.ide.get_primary_sensor_data(doc=doc.channels[acc_id],
                                               measurement_type="acceleration",
                                               time_mode="datetime")

    try:
        temp_df = endaq.ide.get_primary_sensor_data(doc=doc.channels[temp_id],
                                                    measurement_type="temperature",
                                                    time_mode="datetime")
    except:
        temp_df = None
    ...
    with tab4:
        ...
        fig.add_trace(go.Scatter(x=an.index, y=an["X (40g)"],
                                 mode="markers", marker=dict(color="red"),
                                 name="Anomaly"))

        st.plotly_chart(fig, use_container_width=True)
        st.write("Anomalies:", len(an))
