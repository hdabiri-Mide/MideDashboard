...
# ================== PAGE CONFIG ==================
st.set_page_config(layout="wide")
st.title("📊 enDAQ Vibration Dashboard (FFT + PSD + Peak Detection)")
...
resample_rate = st.sidebar.selectbox("Resample Rate", ["None", "1S", "5S", "10S"])
...
peak_height = st.sidebar.number_input("Min Peak Height", value=0.0)
peak_distance = st.sidebar.slider("Min Peak Distance (bins)", 1, 200, 20)
...
def compute_psd(df, column, fs, nperseg):
    return welch(df[column].values, fs=fs, nperseg=nperseg)
...
if uploaded_file is not None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📈 Time Series", "📊 Statistics", "⚡ FFT / PSD + Peaks", "🤖 Anomaly"]
    )
    ...
    with tab3:
        ...
        for col in ["X (40g)", "Y (40g)", "Z (40g)"]:
            ...
            peaks, props = detect_peaks(f_plot, fft_plot)

            # Plot FFT
            fig_fft.add_trace(go.Scatter(x=f_plot, y=fft_plot, name=f"{col} FFT"))
            ...
            # Save peaks
            for p in peaks:
                all_peaks_table.append({
                    "Axis": col,
                    "Frequency (Hz)": f_plot[p],
                    "Magnitude": fft_plot[p]
                })

        fig_fft.update_layout(title="FFT with Peaks", height=500)
        st.plotly_chart(fig_fft, use_container_width=True)

        # Peak table
        st.subheader("Detected Peaks")
        if all_peaks_table:
            st.dataframe(pd.DataFrame(all_peaks_table))
        else:
        ...
        for col in ["X (40g)", "Y (40g)", "Z (40g)"]:
            ...
            peaks, _ = detect_peaks(f_plot, Pxx_plot)

            fig_psd.add_trace(go.Scatter(x=f_plot, y=Pxx_plot, name=f"{col} PSD"))
            ...
            fig_psd.add_trace(go.Scatter(
                ...
                mode='markers',
                marker=dict(color='red'),
                name=f"{col} PSD Peaks"
            ))

        fig_psd.update_layout(title="PSD with Peaks", yaxis_type="log", height=500)
        st.plotly_chart(fig_psd, use_container_width=True)
